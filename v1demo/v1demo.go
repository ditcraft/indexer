...
import (
	...
	"github.com/ditcraft/indexer/smartcontracts_v1/ditDemoCoordinator"
	"github.com/ditcraft/indexer/smartcontracts_v1/ditToken"
...
// Init func
func Init() {
	coordinatorAddress = common.HexToAddress(os.Getenv("CONTRACT_DIT_COORDINATOR_DEMO"))
	knwTokenAddress = common.HexToAddress(os.Getenv("CONTRACT_KNW_TOKEN_DEMO"))
	ditTokenAddress = common.HexToAddress(os.Getenv("CONTRACT_DIT_TOKEN_DEMO"))

	var err error
	ditDemoCoordinatorABI, err = abi.JSON(strings.NewReader(os.Getenv("DIT_DEMO_COORDINATOR_ABI")))
	if err != nil {
		glog.Fatal(err)
	}
...
func handleERC20Transfer(event *types.Log, connection *ethclient.Client) {
	glog.Info("Token Transfer Event")
...
func handleDitDemoCoordinatorInit(event *types.Log, connection *ethclient.Client) {
	...
	for i := 0; i < 3; i++ {
		label, err := ditDemoCoordinatorInstance.GetKnowledgeLabels(nil, initEvent.Repository, big.NewInt(int64(i)))
		if err != nil {
			glog.Error(err)
			return
		}
...
func getDitDemoCoordinatorInstance(_connection *ethclient.Client) (*ditDemoCoordinator.DitDemoCoordinator, error) {
	...
	ditDemoCoordinatorAddressObject := common.HexToAddress(os.Getenv("CONTRACT_DIT_COORDINATOR_DEMO"))
...
func getKNWVotingInstance(_connection *ethclient.Client) (*KNWVoting.KNWVoting, error) {
	knwVotingAddressObject := common.HexToAddress(os.Getenv("CONTRACT_KNW_VOTING_DEMO"))
