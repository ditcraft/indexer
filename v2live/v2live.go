...
import (
	...
	"github.com/ditcraft/indexer/smartcontracts_v2/ditCoordinator"
...
// Init func
func Init() {
	coordinatorAddress = common.HexToAddress(os.Getenv("CONTRACT_DIT_COORDINATOR_LIVE_V2"))
	knwTokenAddress = common.HexToAddress(os.Getenv("CONTRACT_KNW_TOKEN_LIVE_V2"))

	var err error
	ditDemoCoordinatorABI, err = abi.JSON(strings.NewReader(os.Getenv("DIT_LIVE_COORDINATOR_ABI_V2")))
	if err != nil {
		glog.Fatal(err)
	}
...
func GetOldEvents(_fromBlock int64, _toBlock int64) {
	...
	for _fromBlock < _toBlock {
		...
		glog.Info("[OldEvents-Live] Going from " + strconv.Itoa(int(_fromBlock)) + " to " + strconv.Itoa(int(toBlock)))
		...
		if err != nil {
			if strings.Contains(err.Error(), "unexpected EOF") {
				glog.Info("[OldEvents-Live] Got 'unexpected EOF', reducing the offset to " + strconv.Itoa(int(offset)))
				connection, err = getConnection()
				if err != nil {
					glog.Error(err)
				}
	...
	glog.Info("[OldEvents-Live] Finished indexing old blocks for live mode (last block was " + strconv.FormatInt(_toBlock, 10) + ")")
}

// WatchEvents func
func WatchEvents() {
	glog.Info("Starting watcher for live mode...")
	connection, err := getConnection()
	if err != nil {
		glog.Error(err)
	...
	glog.Info("Successfully started watcher for live mode.")
...
func handleNewLabel(event *types.Log, connection *ethclient.Client) {
	...
	err = database.Insert("knowledge_labels_live", newLabel)
	if err != nil {
		glog.Error(err)
	}
...
func handleKNWTokenMint(event *types.Log, connection *ethclient.Client) {
	...
	err = database.Update("knowledge_labels_live", "id", interface{}(int(tokenEvent.Id.Int64())), []string{"total_supply", "last_activity_date"}, []interface{}{floatTotalSupply, time.Unix(int64(block.Time()), 0)})
	if err != nil {
		glog.Error(err)
		return
	}
...
func handleDitDemoCoordinatorInit(event *types.Log, connection *ethclient.Client) {
	...
	ids, err := ditCoordinatorInstance.GetKnowledgeIDs(nil, initEvent.Repository)
	if err != nil {
		glog.Error(err)
		return
	}
...
func getDitCoordinatorInstance(_connection *ethclient.Client) (*ditCoordinator.DitCoordinator, error) {
	...
	ditCoordinatorAddressObject := common.HexToAddress(os.Getenv("CONTRACT_DIT_COORDINATOR_LIVE_V2"))
...
func getKNWVotingInstance(_connection *ethclient.Client) (*KNWVoting.KNWVoting, error) {
	knwVotingAddressObject := common.HexToAddress(os.Getenv("CONTRACT_KNW_VOTING_LIVE_V2"))
